...
WITH temp AS (
    SELECT *,
           CASE
               WHEN 'Procedure' = ANY (
                   SELECT category
                   FROM physician_speciality AS ps2
                   INNER JOIN patient_treatment AS pt
                   ON (ps2.physician_id = pt.physician_id AND ps.physician_id = ps2.physician_id)
                   INNER JOIN event_category AS ec
               ...
               ELSE 0
           END AS valid
    FROM physician_speciality AS ps
)
SELECT speciality, SUM(valid)
FROM temp
GROUP BY  speciality
HAVING SUM(valid) > 0
ORDER BY 2 DESC;
